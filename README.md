...
deck.config {
  ...
  -- 根页面插件排序：'defined'（配置顺序，默认）、'recent'（最近使用）、'most_used'（最多使用）
  plugin_sort = 'defined',
  plugins = {
    -- 远程插件字符串语法
...
插件 spec 可配置 `keys` 字段注册懒加载快捷键。`keys` 是数组，元素格式为 `{ key, callback, desc = ..., path = ... }`；默认绑定到全局快捷键，也可以通过 `path` 指定页面 pattern。按下快捷键时会先执行对应插件的 `setup/config`（即懒加载插件），再调用 `callback`。
